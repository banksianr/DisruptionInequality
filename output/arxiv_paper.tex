...
\subsection{Step 3: The Extreme-Score Problem}

\textbf{This is a critical methodological point.} Disruption scores at both extremes ($D = +1$ and $D = -1$) are mechanically inflated by the interaction between reference counts and citation counts.

\paragraph{Why $D = +1$ is inflated by few references.} A paper with only 3 references makes it easy for citers to avoid all 3 by chance, producing Type~A (``disruptive'') classifications even for ordinary work. Among papers with $\leq 5$ references, 29\% received $D = +1.0$; among those with $\geq 20$ references, only 0.3\% did.

\paragraph{Why $D = -1$ is inflated by few citers and many references.} $D = -1$ means \emph{every single citer} also cites at least one of the focal paper's references. A citer need only overlap with \emph{one} reference to be classified as Type~B (consolidating). With 30+ references, even a citer working in a loosely related area will likely cite at least one of those references. When a paper has only 5--10 citers and many references, the probability that all citers are Type~B approaches certainty---producing $D = -1$ by chance rather than because the paper is truly consolidating. Table~\ref{tab:refbias} shows this interaction clearly: among papers with 36+ references and only 5--10 citers, 30.9\% score $D = -1$.

\begin{table}[H]
\centering
\caption{Interaction of Reference Count and Citation Count on Extreme Scores}
\label{tab:refbias}
\begin{tabular}{@{}llccc@{}}
\toprule
References & Lifetime Citations & $N$ Papers & \% at $D = +1$ & \% at $D = -1$ \\
\midrule
$\leq 5$   & any        & 145   & 29.0\% & 2.1\% \\
10--20     & 5--10      & 126   & ---    & 17.5\% \\
10--20     & 31+        & 406   & ---    & 2.7\% \\
21--35     & 5--10      & 91    & ---    & 26.4\% \\
21--35     & 31+        & 590   & ---    & 2.4\% \\
...
\textbf{Correction applied:} We exclude papers with fewer than 10 references \emph{or} fewer than 20 lifetime citations. This reduces the sample from 3,421 to 2,259 papers across 1,159 authors, but dramatically reduces both artifacts: $D = +1$ drops to 0.0\% and $D = -1$ drops from 7.2\% to 4.4\%.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig3_ref_count_bias.png}
\caption{\textit{Left:} Scatter plot showing that papers with fewer references tend to receive higher disruption scores. \textit{Right:} The percentage of papers at $D = -1$ by lifetime citation count, showing that low-cited papers are mechanically pushed to the extreme negative.}
\label{fig:refbias}
\end{figure}
...
\midrule
Papers analyzed         & 3,421            & 2,259 \\
Authors analyzed        & 1,594            & 1,159 \\
Gini coefficient        & 0.900            & 0.350 \\
Gini interpretation     & ``Extreme''      & ``Moderate'' \\
Top 5\% share           & 50.3\%           & 12.0\% \\
Top 10\% share          & 92.1\%           & 22.2\% \\
vs.\ citation Gini     & Higher (0.66)    & Lower (0.51) \\
Disruption measure      & Count of top-5\% papers & Continuous peak score \\
Reference filter        & None             & $\geq 10$ refs, $\geq 20$ citations \\
\bottomrule
\end{tabular}
\end{table}
...
After filtering to papers with $\geq 10$ references and $\geq 20$ lifetime citations, the distribution of disruption scores shifts substantially. Papers at $D = +1.0$ drop to 0.0\% and $D = -1.0$ drops from 7.2\% to 4.4\% (Figure~\ref{fig:scores}).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig1_score_distribution.png}
\caption{Distribution of disruption scores before (\textit{left}) and after (\textit{right}) applying filters ($\geq 10$ references, $\geq 20$ lifetime citations). The filters eliminate the artificial spike at $D = +1.0$ and substantially reduce $D = -1.0$.}
\label{fig:scores}
\end{figure}
...
Using continuous peak disruption scores, the Gini coefficient is \textbf{0.350} (95\% CI: [0.337, 0.364]). This indicates moderate inequality---some scientists achieve notably higher peak disruption than others, but the distribution is far from the ``extreme concentration'' suggested by the flawed analysis.

For comparison, the Gini for mean citations per paper across the same authors is \textbf{0.506} (95\% CI: [0.462, 0.557]). Disruption is \emph{less} unequally distributed than citations.
...
\caption{\textit{Left:} Distribution of peak disruption scores across 1,159 authors. The median is $-0.33$, meaning the typical scientist's best paper is mildly consolidating. \textit{Right:} Lorenz curves for disruption (Gini = 0.35) and citations (Gini = 0.51). Disruption is more equally distributed than citations.}
\label{fig:lorenz}
\end{figure}
...
Comparing the 106 scientists who produced at least one top-5\% disruptive paper against the 1,053 who did not:
...
\begin{tabular}{@{}lcc@{}}
\toprule
Variable & Disruptors ($n = 106$) & Non-Disruptors ($n = 1{,}053$) \\
\midrule
Team size (avg)          & 4.7   & 5.0 \\
Scored papers            & 2     & 1 \\
Career length (years)    & 18.5  & 14 \\
h-index                  & 23    & 20 \\
Citations per paper      & 53.6  & 56.5 \\
References per paper     & 24.8  & 33.0 \\
\bottomrule
\end{tabular}
\end{table}

The most notable difference is career length: disruptors have a median career span of 18.5 years versus 14 for non-disruptors. This likely reflects a simple statistical regularity---scientists who publish over more years have more chances to produce a high-scoring paper---rather than any deep insight about the nature of disruption. Disruptors also work in slightly smaller teams and, perhaps counterintuitively, receive somewhat \emph{fewer} citations per paper on average.

\subsection{Consistency Across Fields}

The moderate inequality pattern is remarkably consistent across STEM subfields (Table~\ref{tab:fields}). The continuous Gini ranges from 0.31 (Materials Science) to 0.43 (Physics and Astronomy), a narrow band given the diversity of these fields.
...
\midrule
Physics \& Astronomy         & 0.434 & 74  & 4.1\% \\
Computer Science             & 0.408 & 33  & 9.1\% \\
Ag.\ \& Biological Sciences  & 0.389 & 64  & 10.9\% \\
Neuroscience                 & 0.367 & 35  & 2.9\% \\
Health Professions           & 0.359 & 21  & 23.8\% \\
Environmental Science        & 0.357 & 49  & 6.1\% \\
Medicine                     & 0.346 & 338 & 9.5\% \\
Chemistry                    & 0.345 & 50  & 12.0\% \\
Biochem., Genetics \& Mol.\ Bio. & 0.343 & 195 & 7.7\% \\
Engineering                  & 0.332 & 98  & 12.2\% \\
Materials Science            & 0.308 & 47  & 8.5\% \\
\bottomrule
\multicolumn{4}{@{}l}{\footnotesize 11 fields with $\geq 20$ authors and $\geq 1$ disruptor shown (1,004 of 1,159 authors).}\\
...
\midrule
1980--84 & 0.427 & 7.1\%  & 70  & 86 \\
1985--89 & 0.369 & 5.9\%  & 101 & 127 \\
1990--94 & 0.380 & 8.5\%  & 130 & 183 \\
1995--99 & 0.351 & 6.1\%  & 229 & 283 \\
2000--04 & 0.356 & 6.3\%  & 394 & 517 \\
2005--09 & 0.386 & 7.4\%  & 579 & 852 \\
\bottomrule
\end{tabular}
\end{table}
...
Disruptive research is moderately unequally distributed among scientists, with a Gini of 0.35 on continuous peak disruption scores. This is \emph{less} unequal than citation counts (Gini = 0.51), stable across time, and consistent across STEM fields. About 9\% of scientists produce at least one top-5\% disruptive paper, and the top 5\% of scientists account for about 12\% of total disruption---meaningful but not extreme concentration.
...
\paragraph{Lesson 2: Both extreme scores ($D = \pm 1$) are inflated by mechanical artifacts.} Papers citing fewer than 10 references are pushed toward $D = +1$ because citers easily avoid all references by chance. Conversely, papers with many references but few citers are pushed toward $D = -1$ because each citer almost certainly overlaps with at least one of many references. Both artifacts inflate the tails of the disruption distribution and can misclassify papers. These biases likely affect population-level analyses that do not control for reference and citation counts.
...
Our corrected findings are consistent with \citet{park2023}'s observation that most papers are consolidating rather than disruptive. The moderate inequality we observe (Gini = 0.35) is lower than the inequality of citations (Gini = 0.51), suggesting that the capacity to produce \emph{some} disruptive work is more broadly distributed among scientists than the capacity to accumulate citations. This may reflect the fact that disruption and prestige are partially orthogonal: disruptive papers may be controversial, slow to accumulate citations, or published in unexpected venues \citep{wu2019}.

\subsection{Limitations}

\begin{itemize}
    \item \textbf{Sample size}: Our disruption scores cover 2,259 papers across 1,159 authors---far smaller than population-level studies. Results may not generalize to all of science.
...
Disruptive research output is moderately unequally distributed among STEM scientists---less so than citations, stable over time, and consistent across fields. The initially reported ``extreme concentration'' (Gini = 0.90) was an artifact of two methodological choices: applying an inequality measure to a zero-inflated count variable, and failing to control for the mechanical relationship between reference counts, citation counts, and disruption scores. We present these corrections not as a criticism of the disruption index itself, but as a cautionary example of how standard bibliometric analysis steps can compound to produce misleading results. Researchers using the disruption index should (1) filter for minimum reference counts \emph{and} citation counts, and (2) avoid computing Gini coefficients on thresholded count variables.
